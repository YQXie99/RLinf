...
class EnvWorker(Worker):
    ...
    def init_worker(self):
        ...
        elif self.cfg.env.train.simulator_type == "multi_task":
            # hard-coded multi-task interface...
            from rlinf.envs.maniskill.maniskill_env import ManiskillEnv
            from rlinf.envs.libero.libero_env import LiberoEnv
            if self._rank%2 == 0:
                ...
                # Remove keys that shouldn't be in the final config
                merged_dict.pop("maniskill", None)
                merged_dict.pop("libero", None)
                merged_dict.pop("simulator_type", None)
                self._set_init_params_num_envs(merged_dict, mode="train")
                ...
                if not only_eval:
                    self.simulator_list.append(
                        EnvManager(
                            merged_cfg,
                            rank=self._rank,
                            seed_offset=self._rank * self.stage_num,
                            total_num_processes=self._world_size * self.stage_num,
                            env_cls=ManiskillEnv,
                            enable_offload=enable_offload,
                        )
                ...
                if self.cfg.runner.val_check_interval > 0 or only_eval:
                    ...
                    self._set_init_params_num_envs(merged_eval_dict, mode="eval")
    ...
    def env_interact_step(
        ...
        """
        # For multi_task, determine the actual simulator_type based on rank
        simulator_type = self.cfg.env.train.simulator_type
        if simulator_type == "multi_task":
            # For multi_task, rank % 2 == 0 uses maniskill, else uses libero
            simulator_type = "maniskill" if self._rank % 2 == 0 else "libero"
    ...
    def _set_init_params_num_envs(self, cfg_dict, mode):
        target_num_envs = (
